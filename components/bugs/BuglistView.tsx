...
import { Calendar, User } from "lucide-react"; // Removed MoreVertical
        ...
        <tbody className="divide-y divide-[#F1F5F9]">
          {bugs.map((bug) => (
            ...
            >
              <td className="px-6 py-4">
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full ${
                    bug.status.toLowerCase() === "new" ? "bg-red-500" : 
                    bug.status.toLowerCase() === "started" ? "bg-blue-500" : 
                    bug.status.toLowerCase() === "resolved" ? "bg-green-500" : "bg-gray-300"
                  }`} />
                  <span className="text-[13px] font-medium text-[#475569]">{bug.title}</span>
                </div>
              </td>
                ...
                <div className="flex -space-x-2">
                  {Array.isArray(bug.assignees) && bug.assignees.length > 0 ? (
                    bug.assignees.slice(0, 3).map((u, i) => (
                      <div key={i} title={u.name} className="w-7 h-7 rounded-full bg-[#1E293B] border-2 border-white flex items-center justify-center text-white text-[9px] font-bold">
              ...
              {/* FEATURE: Sentiment Output instead of Action Button */}
              <td className="px-6 py-4 text-center">
                {bug.sentiment && (
                  <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full ${bug.sentiment.bg} ${bug.sentiment.color}`}>
                    {bug.sentiment.icon}
